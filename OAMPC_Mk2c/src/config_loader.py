"""Configuration loading utilities for the Mk2c QLabs OAMPC project."""
...
def load_config(config_path: str | Path | None = None) -> dict[str, Any]:
    root = project_root()
    path = Path(config_path).expanduser().resolve() if config_path is not None else root / "oampc_config_mk2c.yaml"
    if not path.exists():
...
def load_obstacle_config(obstacle_config_path: str | Path | None = None) -> dict[str, Any]:
    ...
    path = (
        ...
        else root / "obstacle_config_mk2c.yaml"
    )
    if not path.exists():
