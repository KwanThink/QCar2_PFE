"""Configuration loading utilities for the Mk2b QLabs OAMPC project."""
...
def validate_obstacle_avoidance(config: dict[str, Any]) -> None:
    ...
    activation_config = obstacle_config.get("activation", {})
    for key in ("radius", "field_of_view", "max_active_obstacles"):
        if key not in activation_config:
            raise ValueError(f"obstacle_avoidance.activation.{key} is required.")
    if float(activation_config["radius"]) <= 0.0:
        raise ValueError("obstacle_avoidance.activation.radius must be positive.")
    field_of_view = float(activation_config["field_of_view"])
    if field_of_view <= 0.0 or field_of_view > 360.0:
        raise ValueError("obstacle_avoidance.activation.field_of_view must be in (0, 360].")
    if int(activation_config["max_active_obstacles"]) <= 0:
        raise ValueError("obstacle_avoidance.activation.max_active_obstacles must be positive.")
...
def load_config(config_path: str | Path | None = None) -> dict[str, Any]:
    root = project_root()
    path = Path(config_path).expanduser().resolve() if config_path is not None else root / "oampc_config_mk2b.yaml"
    if not path.exists():
...
def load_obstacle_config(obstacle_config_path: str | Path | None = None) -> dict[str, Any]:
    ...
    path = (
        ...
        else root / "obstacle_config_mk2b.yaml"
    )
    if not path.exists():
