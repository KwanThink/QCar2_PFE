"""Gurobi flat-coordinate OAMPC solver for QCar2 Mk1 tracking."""
...
# Solve a flat-coordinate Cartesian OAMPC QP with a persistent Gurobi template.
class GurobiFlatCoordinateOAMPC:
    ...
    # Reset any optional warm-start information before a new tracking phase starts.
    ...
    def solve(self, current_state: np.ndarray, reference_state: np.ndarray, reference_input: np.ndarray | None = None) -> SolverResult:
        ...
        return SolverResult(delta=delta, ax=ax, v1=v1, v2=v2, status=status, success=success, solve_time=solve_time)
