...
class GurobiFlatCoordinateOAMPC:
    ...
    # Enable Big-M binaries for active obstacles and fix inactive obstacle binaries to zero.
    def _set_active_obstacle_ids(self, active_obstacle_ids: set[int]) -> list[dict[str, Any]]:
        active_obstacles: list[dict[str, Any]] = []
        transition_count = max(0, self.horizon_steps - 1)
        ...
        for obstacle_slot, obstacle in enumerate(self.config_obstacles):
            ...
            for step in range(self.horizon_steps):
                ...
                for edge_slot in range(self.number_of_template_edges):
                    ...
                    if is_active and edge_slot < edge_count:
                        alpha_variable.UB = 1.0
                    else:
                        alpha_variable.UB = 0.0

            for transition in range(transition_count):
                beta_sum_constraint = self.beta_sum_constraints.get((obstacle_slot, transition))
                if beta_sum_constraint is not None:
                    beta_sum_constraint.RHS = 1.0 if is_active else 0.0

                for edge_slot in range(self.number_of_template_edges):
                    beta_variable = self.beta[obstacle_slot, edge_slot, transition]
