"""Gurobi flat-coordinate OAMPC MIQP solver for QCar2 Mk2a tracking."""
...
class GurobiFlatCoordinateOAMPC:
    ...
    def __init__(self, config: dict[str, Any], obstacle_file_config: dict[str, Any]):
        ...
        self._previous_active_obstacle_ids: set[int] | None = None

    # Return active obstacle IDs from the current solve request.
    def _active_obstacle_ids(self, obstacle_list: list[dict[str, Any]] | None) -> set[int]:
        if obstacle_list is None:
            return {int(obstacle["obstacle_id"]) for obstacle in self.config_obstacles}
        return {int(obstacle["obstacle_id"]) for obstacle in obstacle_list}

    # Enable active obstacle constraints and fix all inactive binary variables to zero.
    def _set_active_obstacle_ids(self, active_obstacle_ids: set[int]) -> list[dict[str, Any]]:
        ...
        for obstacle_slot, obstacle in enumerate(self.config_obstacles):
            ...
            is_active = obstacle_id in active_obstacle_ids
            if is_active:
                active_obstacles.append(obstacle)

            for step in range(self.horizon_steps):
                sum_constraint = self.obstacle_sum_constraints.get((obstacle_slot, step))
                if sum_constraint is not None:
                    sum_constraint.RHS = 1.0 if is_active else 0.0
                ...
                for edge_slot in range(self.number_of_template_edges):
                    ...
                    alpha_variable.UB = 1.0 if is_active and edge_slot < edge_count else 0.0
    ...
    def reset_warm_start(self) -> None:
        ...
        self._previous_active_obstacle_ids = None
    ...
    def _warm_start_is_available(self) -> bool:
        return (
            ...
            and self._previous_active_obstacle_ids is not None
        )

    # Save the current usable solution for the next OAMPC iteration.
    def _store_solution_for_warm_start(self, active_obstacle_ids: set[int]) -> None:
        ...
        self._previous_active_obstacle_ids = set(active_obstacle_ids)
    ...
    def _apply_shifted_warm_start(
        ...
        assert self._previous_active_obstacle_ids is not None
        ...
        # Shift binary decisions only when the obstacle remains active. A newly
        # activated obstacle receives no binary start because an all-zero start
        # would violate its obstacle-face sum constraint.
        number_of_obstacles = int(self.template.number_of_config_obstacles)
        for obstacle_slot in range(number_of_obstacles):
            obstacle = self.config_obstacles[obstacle_slot]
            obstacle_id = int(obstacle["obstacle_id"])
            edge_count = int(obstacle["number_of_edges"])
            remains_active = (
                obstacle_id in active_obstacle_ids
                and obstacle_id in self._previous_active_obstacle_ids
            )

            for edge_slot in range(self.number_of_template_edges):
                valid_active_edge = obstacle_id in active_obstacle_ids and edge_slot < edge_count
                for step in range(self.horizon_steps):
                    alpha_variable = self.alpha[obstacle_slot, edge_slot, step]
    ...
    def _result_from_solution(
        ...
        return SolverResult(
            ...
            binary_variables=self.horizon_steps * len(active_obstacles) * active_edges,
    ...
    def solve(
        ...
        active_obstacle_ids = self._active_obstacle_ids(obstacle_list)
        active_obstacles = self._set_active_obstacle_ids(active_obstacle_ids)
        _ = reference_input
        ...
        self._apply_shifted_warm_start(z0, active_obstacle_ids)
        ...
        if success:
            self._store_solution_for_warm_start(active_obstacle_ids)
        else:
