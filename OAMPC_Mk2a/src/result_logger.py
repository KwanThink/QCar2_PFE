"""Result logging for QCar2 OAMPC Mk2a runs."""
...
class ResultLogger:
    # Initialize the logger with code-level defaults for OAMPC Mk2a runs.
    def __init__(self, config: dict[str, Any], obstacle_file_config: dict[str, Any]):
        ...
        self.run_prefix = "run_ObsAv_Mk2a_"
    ...
    def copy_used_config(self) -> None:
        ...
        if config_path.exists():
            shutil.copy2(config_path, self.run_folder / "config_used_mk2a.yaml")
        obstacle_config_path = Path(self.obstacle_file_config["_config_path"])
        if obstacle_config_path.exists():
            shutil.copy2(obstacle_config_path, self.run_folder / "obstacle_config_used_mk2a.yaml")
    ...
    def record(
        ...
        row = {
            ...
            "binary_variables": int(binary_variables),
            "total_binary_variables": int(total_binary_variables),
        }
    ...
    def _save_csv_files(self) -> None:
        ...
        write_dict_csv(self.run_folder / "states_mk2a.csv", self.rows, ["t", "X", "Y", "psi", "vx"])
        write_dict_csv(self.run_folder / "controls_mk2a.csv", self.rows, ["t", "vx_cmd", "delta_cmd", "ax_cmd"])
        write_dict_csv(self.run_folder / "reference_used_mk2a.csv", self.rows, ["t", "ref_index", "X_ref", "Y_ref", "psi_ref", "vx_ref"])
        write_dict_csv(self.run_folder / "reference_controls_mk2a.csv", self.rows, ["t", "delta_ref", "ax_ref"])
        write_dict_csv(self.run_folder / "state_error_mk2a.csv", self.rows, ["t", "X_e", "Y_e", "Psi_e", "vx_e"])
        write_dict_csv(self.run_folder / "input_error_mk2a.csv", self.rows, ["t", "delta_e", "ax_e"])
        write_dict_csv(self.run_folder / "control_error_mk2a.csv", self.rows, ["t", "delta_e", "ax_e"])
        write_dict_csv(self.run_folder / "tracking_error_mk2a.csv", self.rows, ["t", "position_error", "yaw_error", "speed_error"])
        write_dict_csv(
            self.run_folder / "solve_times_mk2a.csv",
            self.rows,
            [
                "t",
                "solve_time",
                ...
                "active_edges",
                "binary_variables",
                "total_binary_variables",
            ],
        )
        write_dict_csv(self.run_folder / "trajectory_reference_mk2a.csv", self.rows, ["time", "reference_index", "X_ref", "Y_ref", "psi_ref", "vx_ref", "delta_ref", "ax_ref"])
        write_dict_csv(
            self.run_folder / "tracking_log_mk2a.csv",
            self.rows,
            [
                "time",
                "reference_index",
        ...
        write_dict_csv(
            self.run_folder / "virtual_input_mk2a.csv",
            self.rows,
            [
                "time",
                "reference_index",
        ...
        write_dict_csv(
            self.run_folder / "obstacle_hulls_mk2a.csv",
            self.obstacle_rows,
    ...
    def _save_plots(self) -> None:
        ...
        self._plot_xy(plt, data, "trajectory_tracking_mk2a.svg")
        ...
        self._plot_series(plt, data, control_signals, "controls_mk2a.svg", "QCar2 Controls")
        self._plot_series(plt, data, control_signals, "control_inputs_mk2a.svg", "QCar2 Controls")
        self._plot_reference_series(
            plt,
            data,
            [
        ...
        self._plot_series(
            ...
            "input_error_mk2a.svg",
            "QCar2 Input Error",
        )
        self._plot_solve_time(plt, data, "solve_time_mk2a.svg")
        self._plot_active_obstacles_from_solve_time_csv(plt, "active_obstacles.svg")
        self._plot_virtual_input(plt, data, "virtual_input_mk2a.svg")
    ...
    # Save the number of active obstacles over time from the solve-time CSV file.
    def _plot_active_obstacles_from_solve_time_csv(self, plt: Any, filename: str) -> None:
        assert self.run_folder is not None
        solve_time_csv_path = self.run_folder / "solve_times_mk2a.csv"
        if not solve_time_csv_path.exists():
            return

        time_values: list[float] = []
        active_obstacle_values: list[int] = []
        with solve_time_csv_path.open("r", newline="", encoding="utf-8") as file:
            reader = csv.DictReader(file)
            if reader.fieldnames is None or "t" not in reader.fieldnames or "active_obstacles" not in reader.fieldnames:
                return
            for row in reader:
                try:
                    time_values.append(float(row["t"]))
                    active_obstacle_values.append(int(float(row["active_obstacles"])))
                except (TypeError, ValueError):
                    continue
        ...
        active_obstacles = np.asarray(active_obstacle_values, dtype=int)

        fig, axis = plt.subplots(1, 1, figsize=(7.0, 3.2))
        axis.step(time_array, active_obstacles, where="post", label="Active obstacles")
        axis.set_title("Active Obstacles")
        axis.set_xlabel("t (s)")
        axis.set_ylabel("Number of active obstacles")
        maximum_active_obstacles = int(np.max(active_obstacles))
        axis.set_yticks(np.arange(0, maximum_active_obstacles + 1, 1))
        axis.grid(True)
        axis.legend()
        fig.tight_layout()
        fig.savefig(self.run_folder / filename, format="svg")
