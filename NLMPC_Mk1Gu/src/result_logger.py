"""Result logging for QCar2 NLMPC Mk1Gu runs."""
...
# Create a new numbered folder inside the result directory.
def create_numbered_run_folder(results_dir: Path, run_prefix: str) -> Path:
    ...
    for child in results_dir.iterdir():
        if child.is_dir() and child.name.startswith(run_prefix):
            suffix = child.name[len(run_prefix) :]
            if suffix.isdigit():
                existing_numbers.append(int(suffix))
    next_number = max(existing_numbers, default=0) + 1
    run_folder = results_dir / f"{run_prefix}{next_number}"
...
class ResultLogger:
    # Initialize the logger with code-level defaults for Mk1Gu runs.
    def __init__(self, config: dict[str, Any]):
        self.config = config
        self.enabled = True
        self.save_plots = True
        self.results_dir = Path(config["_resolved_paths"]["results_dir"])
        self.run_prefix = "run_Mk1Gu_"
    ...
    def copy_used_config(self) -> None:
        ...
        if config_path.exists():
            shutil.copy2(config_path, self.run_folder / "config_info.yaml")
    ...
    # Save all required SVG result plots.
    ...
    def _constraint_limits(self, key: str) -> tuple[float, float] | None:
        constraints = self.config["nlmpc"]["constraints"]
        limit_map = {
            "vx": ("vx_min", "vx_max"),
