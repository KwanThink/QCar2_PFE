"""Configuration loading utilities for the Mk1Gu QLabs NLMPC project."""
...
def load_config(config_path: str | Path | None = None) -> dict[str, Any]:
    root = project_root()
    path = Path(config_path).expanduser().resolve() if config_path is not None else root / "nlmpc_config_mk1gu.yaml"
    if not path.exists():
